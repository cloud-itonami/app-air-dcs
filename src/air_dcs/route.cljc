(ns air-dcs.route
  ...
  build, or a network. `air-dcs.worker` is the only namespace that touches
...
(def nsid-prefix
  "air-dcs の XRPC 名前空間。3 箇所が同じ値を宣言しているのを 1 箇所に集めた:
  `kotodama.jsonld` の `triggers.subscribeRepos.collections`
  (`com.etzhayyim.apps.airDcs.operationalEvent`)、撤去した `src/app.ts` の
  `NSID_PREFIX`、そして `kotoba/src/registry.ts` の collection 名。

  **この prefix は表示のためだけに使う。** 中継そのものは、deploy されていた
  SvelteKit の route と同じく prefix を検査しない —— allowlist を足すのは
  移行ではなく新しい方針である。"
  "com.etzhayyim.apps.airDcs.")

(def actor-did
  "`kotodama.jsonld` の `@id`。ここに写しているのは `/health` が名乗るためで、
  正本はあちら側。**この DID は解決しない** —— `did:web` は
  `https://air-dcs.etzhayyim.com/.well-known/did.json` を要求するが、その host は
  NXDOMAIN（2026-08-18 実測）。移行はそれを直さない。"
  "did:web:air-dcs.etzhayyim.com")

(def default-nanoid
  "撤去した `src/app.ts` が `env.APP_NANOID ?? \"a1rd3cs0\"` と書いていた既定値。
...
(def routes
  ...
  docs/adr/0001 recorded was a page printing `Routes 0` / `No public route is
  declared` beside a `wrangler.jsonc` declaring two routes and eight vars."
...
(defn capability-nsids
  ...
  decode（JSON 文字列 → seq）は `air-dcs.worker` の側にある。**ここは形だけを

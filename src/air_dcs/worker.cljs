(ns air-dcs.worker
  ...
  ここには判断を置かない —— どのハンドラが答えるかは `air-dcs.route/dispatch`
  が決め、ページの中身は `air-dcs.view` が組む。どちらも `.cljc` なので、
  ブラウザもビルドも無しにテストできる。

  wrangler.jsonc の `main` は `dist/worker.js` を指し、それはこの名前空間を
  コンパイルしたものである。移行前は `svelte/.svelte-kit/cloudflare/_worker.js`
  —— tracked file が 1 件も一致せず、tree の何もそれをビルドしない —— を指して
  いて、読み手が開く `src/app.ts` はどの tracked file からも参照されていなかった
  （docs/adr/0001）。

  `aget` を使うのは `:advanced-optimization` 下で env のキーが潰れないため
  （先例 `listingops.edge.worker` と同じ約束）。"
  (:require [air-dcs.route :as route]
...
(defn- env->map
  ...
  （唯一の例外は `APP_CAPABILITIES` で、これは値そのものが公開の宣言である）。"
  [env]
  (if env
